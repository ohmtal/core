...
// NO Splash please $Core::splashWindowImage = "wtp/art/gui/OGS_bluesplash.png";
...
function createCanvas(%windowTitle)
{
   ...
   {
      displayWindow = false; //$platform !$= "windows";
   };
...
if (isToolBuild() && !$isDedicated && !$Client::noTools) {
    $userDirs = $userDirs SPC "tools";
}


//$userDirs =  $userDirs SPC "core"; 
...
function parseArgs()
{
   ...
   // been parsed.
//does not exists   Parent::onParseArgs();
}
...
{  
   ...
   for (%file = findFirstFileMultiExpr(%pattern); %file !$= ""; 
...
{
 // sanity
  for (%i = 0; %i < getWordCount( %dirPath) ; %i++)
  {
      %token = getWord(%dirPath, %i);
      if (!IsDirectory(%token))
      {
         error("Directory" SPC %token SPC "does not exsists!");
         quit();
         return;
      }
  }
